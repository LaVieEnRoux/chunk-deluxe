...
def EM(bitext):
  ...
  for (f, e) in bitext:
    ...
    # Merge alingments
    for i, j in enumerate(a_f):
      if i == a_e[j] and abs(i-j) <= 0.6*(len(a_f)+len(a_e))/2:  
        ...
        for (k, e_k) in enumerate(e):
          ...
          if post > bestp:
            bestp = post
            bestk = k
        if abs(bestk-i) <= 0.2 *(len(a_f)+len(a_e))/2 and bestp > nullWeight:
  ...
  #   # Alignment for F based on posterior
  #   for (i, f_i) in enumerate(f):
  #     bestp = 0
  #     bestj = 0
  #     for (j, e_j) in enumerate(e):
  #       post = float(a_f_e_count[(f_i, e_j)])/float(fe_count[(f_i, e_j)])
  #       if post > bestp:
  #         bestp = post
  #         bestj = j
  #     a_f[i] = bestj

  #   # Alignment for E based on posterior
  #   for (i, e_i) in enumerate(e):
  #     bestp = 0
  #     bestj = 0
  #     for (j, f_j) in enumerate(f):
  #       post = float(a_e_f_count[(e_i, f_j)])/float(ef_count[(e_i, f_j)])
  #       if post > bestp:
  #         bestp = post
  #         bestj = j
  #     a_e[i] = bestj
  ...
  #       sys.stdout.write("%i-%i " % (i, j))
    ...
    # # Merge alingments
    # for i, j in enumerate(a_f):
    #     if i == a_e[j] and np.sqrt(p_f[i] * p_e[j]) > nullWeight:  
    #       sys.stdout.write("%i-%i " % (i, j))
    #       baseline = max(p_f[i], p_e[j])
    #       for m, n in enumerate(a_e):
    #         if n == i and p_e[m] > baseline:
    #             sys.stdout.write("%i-%i " % (i, m))
        # else:
        #   bestm = j
        #   bestp = p_f[i]
        #   for m, n in enumerate(a_e):
        #     if n == i and p_e[m] > bestp:
        #       bestp = p_e[m]
        #       bestm = m    
        #   if bestp > nullWeight:
        #     sys.stdout.write("%i-%i " % (i, bestm))
